  ...
  async input => {
    ...
    try {
      ...
      try {
        ...
        return output || { tafsir: `Error fetching Tafsir: ${errorMessage}` };
      } catch (promptError: unknown) {
         console.error('Error generating Tafsir from prompt:', promptError);
         const promptErrorMessage = promptError instanceof Error ? promptError.message : 'Unknown error';
         return {
            tafsir: `Error fetching Tafsir: ${promptErrorMessage}`,
         }
      }
    }
  }
